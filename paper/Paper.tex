...
\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Refraction index as a function of wavelength. For wavelengths in the emission range of TPB (400–600 nm) the refraction index is practically ﬂat, and has a value of 1.4 \cite{GomezCadenas2016}.}
...
\caption{Energy calibration from the regenerated Geant4 campaign (50--1000~GeV). Total detected photons follow $N = 9683 \times E$~photons~GeV$^{-1}$ (filter OFF, $R^2>0.999$); per-photon CaF$_2$/MgF$_2$ transmission reduces the effective yield to $\sim 6920 \times E$ (filter ON).}
...
\begin{figure}[htbp]
    ...
    \caption{SiPM hits density maps for a 200~GeV gamma ray incident vertically on the center of the top surface (regenerated Geant4 campaign). The directional pattern is energy-independent; the peak-to-mean occupancy factor for this event is $\sim$2.1$\times$.}
    \label{fig:verticaldensity}
\end{figure}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{recon_examples.png}
    \caption{Route~A reconstructed shower axis (red) compared with Monte Carlo truth (blue dashed) for vertical and tilted 200~GeV events. The luminous axis is inferred from the top--bottom brightness-weighted centroid chord.}
...
We implement a two-stage reconstruction pipeline on per-SiPM photon counts (18$\times$18 per face, filter-weighted detection efficiency in simulation). \textbf{Route~A (geometric)} computes brightness-weighted centroids on the top and bottom faces and defines the shower axis along their connecting chord. \textbf{Route~B (ML)} trains a gradient-boosted regressor on 20 engineered features (face photon fractions, centroid offsets, and pair asymmetries) using the direction-scan Monte Carlo as training data. Both routes output a unit vector $\hat{d}$ compared to the primary direction via the opening angle $\Delta\theta = \arccos(\hat{d}\cdot\hat{d}_\mathrm{true})$; we quote $\sigma_{68}$, the 68th percentile of $\Delta\theta$.

For vertical incidence ($\theta=0^\circ$) Route~A achieves $\sigma_{68}\approx 0.13^\circ$ ($n=36$) on the 216-event direction-scan campaign. For inclined primaries the chord method systematically underestimates the tilt ($\sigma_{68}\approx\theta$, e.g.\ $5.1^\circ$ at $\theta=5^\circ$, $45.0^\circ$ at $\theta=45^\circ$). Route~B improves inclined response on held-out test events ($\sigma_{68}\approx 16.5^\circ$ overall, $n_\mathrm{test}=44$) but remains limited by the present training statistics; larger campaigns and direct $6\times18\times18$ map regression are planned.
...
\end{equation}
with fitted parameters \(L_\infty = 666~\mathrm{mm}\) (90\% containment depth $L_{90}$ along the primary direction from longitudinal $dE/dz$ profiles). Measured $L_{90}$ values saturate near $\sim$670~mm for 50--500~GeV $\gamma$ rays in the 1~m cube; box-truncated events are flagged in the events ntuple.  
...
\caption{SiPM saturation energy for the baseline 18$\times$18$\times$6 array using regenerated calibration ($6920$~photons~GeV$^{-1}$ filter ON), peak factor $2.07\times$, PDE~$=0.25$, and $\eta=0.2$, for three microcell densities.}
...
The geometric effective area $A_\mathrm{eff}$ for normally incident $\gamma$ rays is estimated from a plane-source Monte Carlo (1.6$\times$1.6~m$^2$ throw area, 200~GeV, 5064 primaries). Interacting events are identified by LXe energy deposition above threshold; $A_\mathrm{eff} = (N_\mathrm{int}/N_\mathrm{throw})\,A_\mathrm{throw} \approx 1.04$~m$^2$ ($2057/5064$ interacting, Figure~\ref{fig:effective_area}).
...
\midrule
Energy range & 20~MeV--300~GeV & 20~GeV--300~TeV & 50~GeV--1.75~TeV$^{\rm a}$ \\
...
We report a reproducible Geant4~11.3.2 simulation of a 1~m$^3$ liquid-xenon Cherenkov telescope with 1{,}944 interior SiPMs. The regenerated pipeline yields $9683$ detected photons per GeV (filter OFF), $6920$ per GeV (filter ON), $\sigma_E/E \approx 0.50\%$ at 215~GeV, a filter transmission of $\sim 71\%$, and a SiPM peak factor of $2.07\pm 0.45\times$, implying $E_{\rm sat}\approx 1.75$~TeV for baseline 14{,}400-cell devices. Route~A geometric reconstruction achieves $\sigma_{68}\approx 0.13^\circ$ for vertical incidence; effective area at 200~GeV is $\approx 1.04$~m$^2$. These self-consistent numbers replace earlier draft metrics that could not be reproduced from the archived code.
