...
\author[0009-0001-8337-6163]{Qiming Tian}
...
\begin{abstract}
We describe a Geant4 simulation of a cubic liquid xenon (LXe) Cherenkov telescope read out by 1{,}944 interior silicon photomultipliers (18$\times$18 per face). Electromagnetic showers from \qtyrange{50}{1000}{\GeV} $\gamma$ rays yield a detected photon flux of $9683$~photons~GeV$^{-1}$ without optical filtering and $\sim 6920$~photons~GeV$^{-1}$ after CaF$_2$/MgF$_2$ filter transmission ($\bar{T}\approx 0.71$). Photon counting gives $\sigma_E/E \approx 0.50\%$ at 215~GeV, consistent with Poisson statistics. For normally incident $\gamma$ rays, the simulated effective area is $A_\mathrm{eff} \approx 1.0$~m$^2$ between 100 and 1000~GeV and varies by $\lesssim 15\%$ for incidence angles up to $45^\circ$ at 200~GeV. Cherenkov maps exhibit a peak-to-mean SiPM occupancy of $2.07\pm 0.45$, implying a readout saturation limit $E_{\rm sat}\approx 1.75$~TeV for 14{,}400-cell SiPMs; six-face template matching achieves $\sigma_{68}=0.24^\circ$ overall at 30~GeV, with all sampled polar angles below $0.5^\circ$. These results characterize the response of an all-sky LXe Cherenkov concept in the multi-GeV to TeV regime and motivate comparison with \textit{Fermi}-LAT and ground-based Cherenkov arrays.
\end{abstract}

\keywords{Dark Matter (353) --- Gamma-ray telescopes (634) --- High energy astrophysics (739) --- Particle astrophysics (96)}
...
All results reported here are obtained with Geant4~11.3.2 \cite{Agostinelli2003,Allison2016,Tian_NextGenerationGammaRayTelescope_2025}. The detector geometry is a 1.0~m cubic LXe volume inside a 1.02~m aluminum shell. LXe optical properties follow literature Sellmeier dispersion, Seidel absorption, and Aprile/Grace Rayleigh data. SiPM arrays cover all six faces (18$\times$18 per face, 1~cm$^2$ pixels on 5~cm pitch; 1{,}944 channels total). Physics uses \texttt{G4EmStandardPhysics\_option4} plus \texttt{G4OpticalPhysics}; primaries are generated with \texttt{G4GeneralParticleSource} macros. Event-level outputs are written to CSV ntuples and analyzed offline to produce the figures and performance metrics quoted below.
\begin{figure}[h!]
\centering
\begin{tikzpicture}
\begin{axis}[
...
\begin{figure}[htbp]
    ...
    \end{axis}
    \end{tikzpicture}
    \caption{Refractive index of LXe in the VUV Cherenkov band (160--210~nm). The plot axis is restricted to the steep short-wavelength region relevant to Cherenkov threshold and dispersion in simulation. In the separate TPB wavelength-shifter emission band (400--600~nm), the index is nearly flat at $n\approx 1.4$ \cite{GomezCadenas2016}; that plateau is omitted here for clarity.}
...
\]
As an example, at 300~GeV with \(E_\mathrm{dep} = 300~\mathrm{GeV} = 3\times10^{11}~\mathrm{eV}\) and \(W_\mathrm{photon} \approx 13.8~\mathrm{eV/photon}\) \cite{DokeMasuda2002}, a fully absorbed gamma ray would produce
\[
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{direction_six_faces_pair.png}
    \caption{Detected photon density on the six instrumented faces of the 1~m$^3$ LXe cube for two 30~GeV Monte Carlo showers: normally incident ($\theta=0^\circ$, top) and inclined ($\theta=10^\circ$, bottom). Electromagnetic shower development and optical transport spread Cherenkov emission into a diffuse face-wise pattern rather than a sharp ring. For inclined primaries the blob centroid shifts and the relative illumination of opposite faces change coherently across the cube; these inter-face correlations carry the directional information used in reconstruction.}
    \label{fig:direction_six_faces}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{recon_examples_route_d.png}
    \caption{Reconstructed primary direction (solid) and Monte Carlo truth (dashed) for representative 30~GeV test showers at $\theta=0^\circ$ (left) and $\theta=45^\circ$ (right). Unit vectors are shown from the cube center; the algorithm returns the incident direction only. Opening angles for these events are given in the panel titles.}
    \label{fig:direction_recon_examples}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{direction_reco_residuals.png}
    \caption{Distribution of reconstruction residuals on a held-out sample ($n=4320$ events, 50\% calibration library / 50\% test). \emph{Left:} opening angle $\Delta\theta=\arccos(\hat{\mathbf{d}}\cdot\mathbf{d}_\mathrm{true})$; the vertical dashed line marks $\sigma_{68}$. \emph{Right:} $\Delta\theta$ versus true polar angle $\theta$ (points) and the $\sigma_{68}$ value in each $\theta$ bin (curve).}
    \label{fig:direction_residuals}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\linewidth]{angular_resolution_route_d.png}
    \caption{Angular resolution $\sigma_{68}$ versus incidence angle $\theta$ at $E_\gamma=30$~GeV from the direction-scan Monte Carlo (144 $(\theta,\phi)$ bins; 80 events per bin at $\theta\le 10^\circ$ and 40 elsewhere; optical filter ON). Overall $\sigma_{68}=0.24^\circ$; the worst sampled bin is $\theta\approx 5^\circ$ at $\sigma_{68}=0.46^\circ$.}
    \label{fig:angular_resolution_route_d}
\end{figure}

Geant4 simulations with the CaF$_2$/MgF$_2$ filter and 25\% SiPM photon detection efficiency show that each gamma-ray shower produces a characteristic six-face photon pattern (Figure~\ref{fig:direction_six_faces}). Multiple Coulomb scattering, chromatic dispersion, and finite shower length smear the ideal Cherenkov cone into a broad ``light blob'' on every face. The incident direction is encoded in the \emph{shape} of this pattern---centroid offsets within each $18\times18$ array, left--right and top--bottom asymmetry, elliptical second moments, and the relative flux on opposing faces---rather than in the absolute photon yield. Because these shape variables scale weakly with energy once the filter response is applied, angular reconstruction can remain useful in the regime where individual SiPM channels begin to saturate.

We reconstruct $\mathbf{d}_\mathrm{true}$ from the $6\times18\times18$ matrix of detected photon counts using template matching on hand-crafted shape features. For each face we compute five zeroth-order descriptors (centroid offsets, LR/TB asymmetry, $\log N_\gamma$) and two second-moment descriptors (principal-axis angle and eccentricity), giving a 47-dimensional event vector. A calibration library is assembled from direction-scan Monte Carlo, retaining up to 20 events per $(\theta,\phi)$ bin. An unknown event is matched to its $k$ nearest library entries in normalized feature space ($k=7$--$11$); their truth directions are combined with Gaussian or inverse-distance weights to yield a unit vector $\hat{\mathbf{d}}$. Near zenith, where asymmetry is small, we give greater weight to a 30-dimensional subset matched with inverse-distance weighting; for inclined showers we favor the full 47-dimensional descriptor with Gaussian weighting. Performance is quantified by the opening angle $\Delta\theta=\arccos(\hat{\mathbf{d}}\cdot\mathbf{d}_\mathrm{true})$ and summarized by $\sigma_{68}$, the 68th percentile of $\Delta\theta$ on held-out data.

The direction-scan campaign at 30~GeV samples six polar angles ($0^\circ$, $5^\circ$, $10^\circ$, $20^\circ$, $30^\circ$, $45^\circ$) and 24 azimuthal bins (8640 events total, with additional statistics at $\theta\le 10^\circ$). With half of the data reserved for testing, we obtain $\sigma_{68}=0.24^\circ$ overall (Figures~\ref{fig:direction_residuals} and~\ref{fig:angular_resolution_route_d}). All sampled $\theta$ bins satisfy $\sigma_{68}<0.5^\circ$; inclined primaries ($\theta\gtrsim 20^\circ$) reach $\sigma_{68}\lesssim 0.01^\circ$. Because reconstruction returns a weighted combination of calibration \emph{directions} rather than a discrete $\theta$ label, events with $\theta$ between calibration grid points are interpolated in feature space (e.g.\ $\theta=7.4^\circ$ is bracketed by the 5$^\circ$ and 10$^\circ$ libraries). Figure~\ref{fig:direction_recon_examples} illustrates representative reconstructed and true directions for vertical and inclined showers.

The calibration library is built at a fixed primary energy, but the shape features used for matching are largely insensitive to absolute photon yield once counts are expressed as face-wise patterns; extending angular calibration to the full mission band therefore requires only a sparse set of anchor energies (e.g.\ 30, 100, and 300~GeV), not a separate direction scan at every integer gigaelectronvolt. Energy measurement, by contrast, uses the total detected photon count and follows the linear calibration of Figure~\ref{fig:energy_calibration}.
...
\frac{\sigma_E}{E_0}.
\end{equation}
For the present dataset, $\sigma_E/E_0 \approx 0.50\%$ at 215~GeV (filter ON, 40 events), consistent with Poisson-limited photon counting once shower and optical transport fluctuations are included (see Figure~\ref{fig:energy_resolution}). The overall trend of linear scaling between Cherenkov photon yield and gamma-ray energy remains valid across the examined energy range.

\begin{figure}[h!]
\centering
\includegraphics[width=0.75\linewidth]{energy_resolution.png}
\caption{Reconstructed energy distribution at 215~GeV from the simulated Monte Carlo campaign (filter ON).}
...
\begin{figure}[htbp]
    ...
    \caption{Electromagnetic shower 90\% containment depth $L_{90}$ vs.\ primary energy from the simulated Geant4 campaign (longitudinal $dE/dz$ profile along the primary direction; box-truncated events excluded from the fit).}
    \label{fig:shower-fit}
\end{figure}
The maximum gamma-ray energy that can be substantially contained within a calorimeter is governed by the longitudinal development of the electromagnetic shower. As a pedagogical upper bound, we first consider a simplified cascade-splitting parameterization in which the average deposition profile follows a Gamma distribution with $\beta\simeq 1$ and $\alpha=\log_2(E_0/E_c)$. This binary-branching toy model illustrates how containment scales with calorimeter depth, but it is more optimistic than electromagnetic-shower profiles in high-$Z$ media summarized in the Particle Data Group review \cite{Workman:481642}.

The average energy deposition of an electromagnetic shower as a function of depth $t$ (in units of radiation lengths $X_0$) is described by the Gamma distribution
\begin{equation}
...
\end{equation}
This equation can be solved numerically within the toy model to obtain a formal upper bound. We evaluate one illustrative case with the following parameters:
\begin{itemize}
...
\end{align}
Converting this to GeV, the toy-model formal limit is
\begin{equation}
E_{\gamma}^{\mathrm{max}} \approx 6.713 \times 10^5~\mathrm{GeV} \approx 671 ~\mathrm{TeV}.
\end{equation}
This value should not be interpreted as the operational energy reach of the 1~m$^3$ instrument; it is the asymptotic ceiling of the simplified parameterization above.

To obtain the empirically relevant longitudinal scale, we performed Geant4 Monte Carlo simulations and analyzed the resulting showers with ParaView. For each incident gamma-ray energy $E$, we recorded the corresponding shower length $L$.
...
\end{equation}
with fitted parameters $L_\infty = 666$~mm (90\% containment depth $L_{90}$ along the primary direction from longitudinal $dE/dz$ profiles). Measured $L_{90}$ values saturate near $\sim$670~mm for vertically incident 50--500~GeV $\gamma$ rays in the 1~m cube; box-truncated events are flagged in the events ntuple.

The simulated $L_{90}$ scale shows that shower cores in the campaign energy range fit within the cubic LXe volume, but this behavior is far below the 671~TeV toy-model ceiling: realistic electromagnetic cascades in xenon develop more gradually in depth than the $\beta\simeq 1$, $\log_2(E_0/E_c)$ parameterization implies. Empirical shower-depth estimates for high-$Z$ media place 95\% longitudinal containment in $\sim 35\,X_0$ of LXe at the few-TeV scale rather than hundreds of TeV \cite{Workman:481642}. For the present concept, the practical high-energy bound is therefore set by SiPM readout saturation ($E_{\rm sat}\approx 1.75$~TeV for baseline devices) and by transverse and longitudinal leakage at inclined incidence, not by the toy-model limit.
...
To quantify this non-uniformity, we analyzed the simulated six-face density maps (Figure~\ref{fig:direction_six_faces}). The peak photon density on the hottest SiPM is $2.07\pm 0.45\times$ the array average (30 events at 1~TeV); this factor decreases mildly with energy as showers smooth out, so it represents a conservative (high) value for the multi-TeV saturation regime. This measured peak factor enters the saturation calculation: if the average number of photons per SiPM is $\bar{N}_{\rm phot}(E_0) = N_{\rm tot}(E_0)/N_{\rm SiPM}$, then the peak SiPM receives
\begin{equation}
N_{\rm peak}(E_0) = 2.07 \times \bar{N}_{\rm phot}(E_0) = 2.07 \times \frac{N_{\rm tot}(E_0)}{N_{\rm SiPM}},
\end{equation}
and saturation occurs when $N_{\rm peak}(E_{\rm sat}) \times {\rm PDE} = \mu_{\max}$. For a given $N_{\rm cell}$ and $\eta$, this yields
\begin{equation}
E_{\rm sat} = \frac{\mu_{\max}/{\rm PDE}}{2.07 \times (N_{\rm tot}/E)/N_{\rm SiPM}} 
= \frac{-N_{\rm cell}\ln(1-\eta)}{{\rm PDE} \times 2.07 \times (N_{\rm tot}/E)/N_{\rm SiPM}}.
\end{equation}
...
The baseline 18$\times$18$\times$6 configuration with current technology achieves $E_{\rm sat} \approx 1.75$~TeV for $\eta=0.2$ ($\sim$12~TeV with 100{,}000-cell and $\sim$53~TeV with 440{,}000-cell SiPMs). Since $E_{\rm sat} \propto N^2$, scaling to a 40$\times$40$\times$6 array (9,600~SiPMs) with the same 14,400-cell devices raises $E_{\rm sat}$ to $\sim$8~TeV, while combining this array size with ultimate-technology SiPMs (440,000~cells) extends the range to $\sim$240~TeV—approaching the performance of ground-based imaging air Cherenkov telescopes. With ultimate SiPM technology the readout saturation limit can thus be pushed toward the hundred-TeV level, still well below the toy-model formal ceiling ($\sim$671~TeV) derived above. The choice of SiPM technology and array density therefore represents a trade-off between cost, data rate, and accessible energy range in the multi-TeV regime relevant to this study.
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{saturation_energy.png}
\caption{SiPM saturation energy for the baseline 18$\times$18$\times$6 array using simulated calibration ($6920$~photons~GeV$^{-1}$ filter ON), peak factor $2.07\times$, PDE~$=0.25$, and $\eta=0.2$, for three microcell densities.}
\label{fig:saturation_vs_array}
\end{figure}

\subsection{Background Rejection}

In the multi-GeV to TeV band, the differential flux of cosmic-ray protons and heavier nuclei exceeds that of astrophysical $\gamma$ rays by factors of $10^3$--$10^5$, depending on energy and galactic latitude \cite{Ackermann2012}. Rejection of this dominant charged-particle background therefore requires an active veto in addition to any shower-shape discrimination available in the LXe volume.

The telescope employs a segmented plastic-scintillator anti-coincidence detector (ACD) modeled on the \textit{Fermi}-LAT design, with panels on all six exterior faces read out by SiPMs. Charged particles traversing the ACD produce a coincident scintillation signal that flags the event for rejection before further reconstruction. Segmentation limits self-veto of neutral $\gamma$ rays caused by escaping shower secondaries, which becomes increasingly important toward the multi-TeV regime.

Shower morphology provides a complementary handle: Monte Carlo proton primaries (FTFP\_BERT) yield broader, more isotropic six-face photon patterns than $\gamma$ rays. The face-anisotropy variable separates the two species with a median proton-to-$\gamma$ ratio of $\sim 2.6$ (Figure~\ref{fig:background_rejection}). This separation is insufficient on its own given the cosmic-ray-to-$\gamma$ flux hierarchy; it is most effective when applied \emph{after} the ACD veto and combined with additional shape variables in a multivariate selection. Full quantification of the integrated rejection factor, including optimization of ACD thresholds to balance proton rejection against $\gamma$-ray efficiency, is reserved for a dedicated simulation campaign.
...
The effective area $A_\mathrm{eff}$ quantifies the collecting power of the instrument for gamma-ray primaries. We estimate it from plane-source Monte Carlo: primaries are sampled uniformly over a 1.6$\times$1.6~m$^2$ square centered above the detector face, and an event is counted as interacting if the total energy deposited in LXe exceeds 1~MeV. With throw area $A_\mathrm{throw} = 2.56$~m$^2$,
\begin{equation}
A_\mathrm{eff} = \frac{N_\mathrm{int}}{N_\mathrm{throw}}\, A_\mathrm{throw}.
\end{equation}

Vertical-incidence campaigns at 100, 300, 500, and 1000~GeV (1200 primaries per energy) give $A_\mathrm{eff} = 1.06$, 1.06, 0.99, and 1.03~m$^2$, respectively (Figure~\ref{fig:effective_area_scan}, left panel). The response is flat to within the Monte Carlo uncertainty ($\lesssim 10\%$): the interaction fraction remains $\sim 0.40$--$0.42$ across the sampled energy range. A high-statistics plane-source run at 200~GeV (5064 primaries) yields $A_\mathrm{eff} = 1.04$~m$^2$, consistent with the energy-scan points.

At fixed 200~GeV energy, tilted plane sources at $\theta = 15^\circ$, $30^\circ$, and $45^\circ$ (600 primaries each) give $A_\mathrm{eff} = 1.07$, 0.93, and 1.13~m$^2$ (Figure~\ref{fig:effective_area_scan}, right panel). The modest variation reflects geometric acceptance of the cubic volume rather than a strong energy-dependent cutoff. For reference, the on-axis effective area of \textit{Fermi}-LAT peaks near 0.8~m$^2$ at a few GeV and falls at higher energies where showers are poorly contained \cite{Atwood2009}; the simulated LXe cube maintains $\sim 1$~m$^2$ acceptance through 1~TeV in this simplified geometry, before readout saturation limits reliable energy measurement above $\sim 1.75$~TeV.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{effective_area_scan.png}
\caption{Simulated effective area vs.\ primary energy (vertical incidence, left) and vs.\ incidence angle at 200~GeV (right). Error bars are not shown; statistical uncertainties on $A_\mathrm{eff}$ are $\sim 3$--$5\%$ for the 1200-event energy points and $\sim 2\%$ for the 5064-event plane-source reference.}
\label{fig:effective_area_scan}
\end{figure}
...
Table~\ref{tab:instrument_compare} summarizes how the simulated LXe Cherenkov concept compares with operating facilities. The cubic geometry accepts gamma rays from essentially all directions within the LXe volume. \textit{Fermi}-LAT maintains a wide survey field of view ($\sim 2.4$~sr) but pair-track angular resolution degrades below $\sim 0.1^\circ$ only above 10~GeV, and calorimeter depth limits containment above a few hundred GeV \cite{Atwood2009,Ackermann2012}. Ground-based CTA achieves superior angular resolution ($\sim 0.05^\circ$) and TeV sensitivity but with a narrow field of view and $\lesssim 15\%$ duty cycle \cite{CTAO_performance_2025}. In simulation, photon-counting energy resolution at 215~GeV approaches the Poisson limit ($\lesssim 1\%$), and the effective area near 1~m$^2$ is comparable to the \textit{Fermi}-LAT on-axis peak while extending to higher energies within the 1~m$^3$ LXe volume; readout saturation presently caps reliable event energy near 1.75~TeV for baseline SiPMs.
...
\midrule
Energy range & 20~MeV--300~GeV & 20~GeV--300~TeV & 50~GeV--1.75~TeV\tablenotemark{a} \\
$A_\mathrm{eff}$ (on-axis) & $\sim 0.8$~m$^2$ @ few GeV & $\sim 10^4$~m$^2$ @ TeV & $\sim 1.0$~m$^2$ @ 0.1--1~TeV \\
$\sigma_E/E$ @ 215~GeV & $\sim 9\%$ @ 1~GeV & moderate & $\sim 0.50\%$ \\
Angular resolution & $\sim 0.6^\circ$ @ 1~GeV & $\sim 0.05^\circ$ & $\sigma_{68}=0.24^\circ$ @ 30~GeV \\
FoV / acceptance & 2.4~sr survey & $\sim 10$~deg & $\lesssim 1~{\rm m}^3$ cube \\
Duty cycle & $>90\%$ & $\lesssim 15\%$ & space (concept) \\
\bottomrule
\end{tabular}
\tablenotetext{a}{Saturation-limited with 14{,}400-cell SiPMs; geometric containment extends higher.}
\end{table}

\subsection{Systematic Uncertainties}

Remaining systematic uncertainties include: (i)~\textit{LXe optical model}---Sellmeier $n(\lambda)$, absorption, and Rayleigh data propagate directly into Cherenkov yield and transport; (ii)~\textit{shower fluctuations}---finite Monte Carlo statistics and cascade variance dominate energy resolution at fixed $E$; (iii)~\textit{SiPM granularity}---1~cm$^2$ pixels and 25\% PDE approximate commercial VUV SiPMs but omit pixel crosstalk and afterpulsing; (iv)~\textit{filter model}---TMM transmission is idealized (no surface roughness or angular dependence); (v)~\textit{geometry leakage}---1~m$^3$ LXe may not fully contain the highest-energy showers; and (vi)~\textit{angular calibration}---present direction reconstruction is validated at 30~GeV and may require energy-dependent calibration libraries at higher energies.
...
The selection of materials for a space-based Liquid Xenon Cherenkov telescope is a critical engineering challenge, demanding a careful balance among cryogenic stability, structural integrity, and the minimization of high-energy gamma-ray interaction. The telescope requires a robust cryogenic design to maintain the LXe medium at a stable \qty{165}{\kelvin} and ensure exceptional temperature uniformity, which is vital for the precise measurement of the medium's refractive index and accurate direction reconstruction.

From a mass-budget perspective, the 1.0~m$^3$ active LXe volume alone contributes approximately \qty{2.95}{\ton} at the nominal liquid density ($\rho_{\rm LXe}\approx \qty{2.95}{\gram\per\cubic\centi\meter}$). Accounting for the inner cryogenic vessel, segmented anti-coincidence panels, titanium structural frame, multi-layer insulation, and pulse-tube cryocooler, the total instrument mass is estimated at $\sim 3.5$--$4$~metric~tons, comparable to the \textit{Fermi} Gamma-ray Space Telescope ($\sim 4.3$~t) \cite{Atwood2009}. This places the concept in the class of flagship astrophysics payloads. Although such masses exceed the dedicated capacity of many historical astronomy launch vehicles, emerging heavy-lift systems---notably SpaceX Starship, with a nominal low-Earth-orbit payload capability exceeding 100~t \cite{SpaceXStarship2020}---substantially relax the launch-mass constraint and enable dedicated deployment of large cryogenic instruments. The associated trade-off is justified by the simulated combination of near-omnidirectional acceptance within the LXe cube and Poisson-limited energy resolution at the percent level.
...
We have simulated a 1~m$^3$ liquid-xenon Cherenkov telescope with 1{,}944 interior SiPMs using Geant4~11.3.2. The instrument yields $9683$ detected photons per GeV without filtering and $6920$ per GeV with the CaF$_2$/MgF$_2$ filter in place ($\bar{T}\approx 0.71$). Photon counting gives $\sigma_E/E \approx 0.50\%$ at 215~GeV. The effective area for vertical $\gamma$ rays is approximately 1~m$^2$ between 100 and 1000~GeV, with only modest dependence on incidence angle at 200~GeV. A peak SiPM occupancy factor of $2.07\pm 0.45$ implies $E_{\rm sat}\approx 1.75$~TeV for baseline 14{,}400-cell devices, and six-face template matching achieves $\sigma_{68}=0.24^\circ$ overall at 30~GeV with all sampled polar angles below $0.5^\circ$. Future work includes sparse multi-energy angular calibration, ACD veto optimization, and experimental validation of the optical filter and SiPM response.
